...
def landing(request):
    ...
    #kundenInLaden = 24
    #vergleichVorwoche = 9
    return render(request, 'index.html', {'gesamtKundenzahl': 55, 'kundenInLaden': 30, 'vergleichVorwoche': -5})
...
def settings(request):
    ...
    updateCam = False
    updateCam = True
    f = open('/home/pi/Skripte/Demo/DemoServer/Settings.json')
    print(type(f))
    rawdata = f.read()
...
def chart_vonbis(start, end):
    ...
    # Richtige Daten eintragen
    data = dbc.query(
        f"select kundenzahl from daten where Date(Datum) BETWEEN date(\"{start}\") AND date(\"{end}\")")
    print(data)
